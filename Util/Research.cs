...
namespace ResearchSync.Util
{
	internal class Research
	{
		...
		{
			var itemIds = CreativeItemSacrificesCatalog.Instance.SacrificeCountNeededByItemId.Keys;
			var current = new Dictionary<int, int>();
			foreach ( var id in itemIds)
			{
				var count = player.Player.creativeTracker.ItemSacrifices.GetSacrificeCount(id);
				if (count > 0)
				{
					current[id] = count;
				}
